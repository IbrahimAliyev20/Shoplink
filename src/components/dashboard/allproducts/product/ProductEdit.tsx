...
const ProductEdit: React.FC = () => {
  ...
  const { mutate: updateProduct, isPending } = useMutation({
    ...
    onError: (error) => {
      toast.error("Məhsul yenilənərkən xəta baş verdi: " + error.message);
    },
  });
    ...
    if (product && categories && !categoriesLoading) {
      ...
      const initialData = {
        ...
        discount_price: product.detail.discount_price
          ? parseFloat(product.detail.discount_price)
          : undefined,
  ...
  useEffect(() => {
    if (watchedNewImages && watchedNewImages.length > 0) {
      const urls = Array.from(watchedNewImages).map((file) =>
  ...
  const handleDeleteExistingImage = (id: number) => {
    ...
    setDeletedImageIds((prev) => {
      ...
      console.log("Updated deletedImageIds:", newIds);
      return newIds;
    });
  ...
  if (isLoading || categoriesLoading || !isFormReady)
    ...
    <div className="min-w-[260px] w-full mx-auto">
      ...
      >
        <fieldset disabled={isPending}>
          <h2 className="text-lg font-medium mb-4 sm:mb-6 break-words">
            {product?.name}
          </h2>
          ...
          <div>
              ...
              <div className="flex flex-col sm:flex-row items-center gap-4">
                {mainImagePreview && (
                  <Image
                    ...
                    width={128}
                    height={128}
                    className="object-cover w-32 h-32 rounded-xl border"
                  />
                )}
                <label
                  htmlFor="main-image-upload"
                  className="w-full bg-[#FBFDFF] border rounded-lg p-4 text-center cursor-pointer"
                >
                  <Image
            ...
            <div className="flex flex-wrap gap-3 sm:gap-4 items-center">
                ...
                <div key={image.id} className="relative group w-32 h-32">
                  ...
                  <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 rounded-xl flex items-start justify-end p-1.5">
                    <button
                      ...
                      className="p-1.5 bg-white/80 rounded-md"
                    >
              ...
              <label
                ...
                className="w-32 h-32 flex flex-col items-center justify-center bg-gray-50 rounded-lg border cursor-pointer"
              >
                <div className="w-8 h-8 rounded-full bg-white border flex items-center justify-center mb-2">
          ...
          <div className="flex justify-end mt-6 sm:mt-10">
            <Button
              type="submit"
              disabled={isPending}
              className="bg-[#E23359] text-white px-6 py-3 rounded-lg font-medium hover:bg-[#E23359]/90 disabled:opacity-50"
            >

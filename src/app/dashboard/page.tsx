...
export default function DashboardPage() {
  ...
  const activeChartData = useMemo(() => {
    switch (timeframe) {
      ...
      default: return monthlyChartData;
    }
  }, [timeframe]);
  ...
  return (
    <div >
      <div className="space-y-8 max-md:space-y-6">
        <div>
          <h1 className="text-2xl font-medium text-gray-900 max-md:text-2xl">Panel</h1>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 max-md:grid-cols-2 max-md:gap-3">
          {dashboardMetrics.map((metric, index) => {
            ...
            return (
              <Card key={index} className="bg-white shadow-none border-gray-200">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 max-md:p-3">
                  <CardTitle className="text-sm font-medium text-gray-600 max-md:text-xs max-md:leading-tight">{metric.title}</CardTitle>
                  <div className={`h-12 w-12 rounded-lg ${colors.icon} flex items-center justify-center max-md:h-8 max-md:w-8 flex-shrink-0`}>
                ...
                <CardContent className="max-md:p-3 max-md:pt-0">
                  <div className="text-2xl font-bold text-gray-900 max-md:text-lg">{metric.value}</div>
                </CardContent>
              </Card>
            );
          })}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 max-md:gap-4">
          <Card className="lg:col-span-2 bg-white shadow-none border-gray-200">
            <CardHeader className="flex flex-row items-center justify-between max-md:p-4">
              <CardTitle className="text-lg font-semibold text-gray-900 max-md:text-base">Gəlir qrafiki</CardTitle>
            ...
            <CardContent className="h-[350px] w-full p-4 max-md:h-[250px] max-md:p-2">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={activeChartData} margin={{ top: 5, right: 20, bottom: 20, left: 30 }}>
                  <defs>
            ...
            <CardHeader className="max-md:p-4">
              <CardTitle className="text-lg font-semibold text-gray-900 max-md:text-base">Son fəaliyyətlər</CardTitle>
            </CardHeader>
            <CardContent className="max-md:p-4 max-md:pt-0">
              <div className="space-y-4 max-md:space-y-3">
                {recentActivities.map((activity, index) => {
                  ...
                  return (
                    <div key={index} className="flex items-start space-x-3 max-md:space-x-2">
                      <div className={`w-8 h-8 ${iconColor} rounded-full flex items-center justify-center flex-shrink-0 max-md:w-6 max-md:h-6`}>
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm text-gray-800 font-medium max-md:text-xs max-md:leading-tight">
                          {activity.company} {activity.description && `— ${activity.description}`}
                        </p>
                        {activity.time && (

...
const CreateProduct = () => {
  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>, type: 'main' | 'additional') => {
    ...
    <div className="mx-auto space-y-6 bg-gray-50/50 p-4 sm:p-6 lg:p-8">
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <fieldset disabled={isPending} className="space-y-6">
          ...
          <Card>
            <CardHeader><CardTitle className="text-2xl font-medium">Əsas məlumatlar</CardTitle></CardHeader>
            <CardContent className="space-y-6">
              <div className="space-y-2">
                <Label htmlFor="product-name">Məhsulun adı</Label>
                <Input id="product-name" placeholder="Məhsulun adını daxil edin" {...register("name", { required: "Ad mütləqdir" })} />
          ...
          <Card>
            <CardHeader><CardTitle className="text-2xl font-medium">Məhsulun şəkli</CardTitle></CardHeader>
            <CardContent className="space-y-6">
                <div>
                    ...
                    {!mainImagePreview ? (
                        ...
                        <div className="mt-2 relative w-32 h-32 border rounded-md">
                            <Image src={mainImagePreview} alt="Əsas şəkil" layout="fill" className="object-cover rounded-md" />
                            <Button type="button" variant="destructive" size="icon" className="absolute -top-2 -right-2 h-6 w-6 rounded-full" onClick={removeMainImage}><X className="h-4 w-4"/></Button>
                        </div>
                    )}
                </div>
                <div>
                        ...
                        <div className="flex flex-wrap gap-4 mt-4">
                            {additionalImagePreviews.map((preview, index) => (
                                <div key={index} className="relative w-32 h-32 border rounded-md">
                                    <Image src={preview} alt={`Əlavə şəkil ${index + 1}`} layout="fill" className="object-cover rounded-md" />
                                    <Button type="button" variant="destructive" size="icon" className="absolute -top-2 -right-2 h-6 w-6 rounded-full" onClick={() => removeAdditionalImage(index)}><X className="h-4 w-4"/></Button>
                                </div>
                            ))}
                        </div>
                    )}
          ...
          <Card>
            <CardHeader><CardTitle className="text-2xl font-medium">Məhsul detalları</CardTitle></CardHeader>
            <CardContent className="space-y-6">
                ...
                <div className="space-y-2">
                    ...
                    <Controller name="description" control={control} render={({ field }) => <RichTextEditor value={field.value} onChange={field.onChange} placeholder="Məhsulun təsvirini daxil edin" />}/>
                </div>
            </CardContent>
          </Card>
          
          <Card>
            <CardHeader><CardTitle className="text-lg font-semibold">SEO</CardTitle></CardHeader>
            <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
              <div className="space-y-2">
                <Label htmlFor="meta_keywords">Səhifə başlığı (Keywords)</Label>
                <Input id="meta_keywords" placeholder="Səhifə başlığı" {...register("meta_keywords")} />
              </div>
              <div className="space-y-2">
          ...
          <div className="flex justify-end gap-4 pt-4">
            <Button type="button" variant="outline" size="lg" className="px-8">Geri</Button>
            <Button type="submit" className="bg-[#FF13F0] hover:bg-[#FF13F0]/90 text-white px-8 cursor-pointer" size="lg" disabled={isPending}>
